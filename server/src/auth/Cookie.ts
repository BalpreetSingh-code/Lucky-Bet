/**
 * Represents an HTTP cookie used for session tracking.
 * Handles value assignment, expiration, and formatting for the `Set-Cookie` header.
 */
export default class Cookie {
	 ...
	 * @param name Name of the cookie (e.g. "session_id").
	 * @param value Value stored in the cookie.
	 * @param expires Expiration duration in milliseconds (defaults to 10 min).
	 * @param httpOnly Prevents client-side JavaScript from accessing the cookie.
	 */
	constructor(
		name: string,
		value: string,
	...
	/**
	 * Gets the expiration timestamp (in ms).
	 * @returns Milliseconds since epoch when the cookie expires.
	 */
	getExpires() {
